...
本テンプレートは美容業種（ネイル・エステ・サロン）向けの架空デモ。

- 業務委託元: 個人副業（ユーザー本人）
- 想定読者: ココナラ閲覧者・小規模事業主
- 派生元: `lp-psychology-template`（同じ構造の汎用版）
...
```bash
cd ~/dev/templates/lp-beauty-template
grep -rn "Salon AOI\|一番\|月限定\|先着\|限定価格\|No\\.1\|人気No\|行動経済学\|心理学\|6要素" src/
# → 0件であれば OK
```

### ローカル動作確認（Mac 負荷注意）
```bash
cd ~/dev/templates/lp-beauty-template && npm run dev
